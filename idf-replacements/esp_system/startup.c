...
// Lua-RTOS pthread init (replaced component, so ESP-IDF's init_pthread CORE-stage function is absent)
...
{
    // All core initialization (heap, newlib, brownout, pthread, flash, efuse,
    // VFS console, etc.) is handled by ESP_SYSTEM_INIT_FN functions registered
    // in their respective components with CORE stage.
    //
    // The Lua-RTOS newlib replacement component registers the following CORE-stage
    // functions via idf-replacements/newlib/init.c:
    //   init_libc       (priority 102) -> esp_libc_init() / esp_newlib_init()
    //   init_libc_stdio (priority 115) -> esp_newlib_init_global_stdio("/dev/console")
    //
    // These run in the correct order inside do_system_init_fn:
    //   priority 100: init_heap       (heap_caps_init)
    //   priority 102: init_libc       (syscall table, _GLOBAL_REENT, newlib locks)
    //   priority 114: init_vfs_console (registers /dev/console in VFS)
    //   priority 115: init_libc_stdio  (opens stdin/stdout/stderr on /dev/console)
    //
    // Do NOT call esp_newlib_init() or esp_newlib_init_global_stdio() here; doing
    // so would cause a second initialization that zeroes __sglue (corrupting the
    // file pool) and leaves _GLOBAL_REENT->_stdout NULL, causing vprintf to crash
    // in main_task with LoadProhibited at EXCVADDR 0x00000008.
...
static void start_cpu0_default(void)
{
    ...
    // Lua-RTOS replaces the ESP-IDF pthread component, so the CORE-stage
    // init_pthread function is absent. Initialize pthread before constructors
    // run, since C++ global constructors may call pthread_key_create.
    _pthread_init();
...
ESP_SYSTEM_INIT_FN(init_components0, SECONDARY, BIT(0), 200)
{
