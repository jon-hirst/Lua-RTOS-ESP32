...
static int _fsync_console(int fd)
{
    if (fd == STDOUT_FILENO || fd == STDERR_FILENO) {
        ...
        esp_rom_output_flush_tx(CONFIG_ESP_CONSOLE_UART_NUM);
#elif defined(CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG)
        esp_rom_output_flush_tx(CONFIG_ESP_ROM_USB_SERIAL_DEVICE_NUM);
#elif defined(CONFIG_ESP_CONSOLE_USB_CDC)
        esp_rom_output_flush_tx(CONFIG_ESP_ROM_USB_OTG_NUM);
#endif
        return 0;
    }
    errno = EBADF;
