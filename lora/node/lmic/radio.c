...
static void IRAM_ATTR writeReg (u1_t addr, u1_t data ) {
    hal_pin_nss(0);
    hal_spi(addr | 0x80);
    hal_spi(data);
    hal_pin_nss(1);
...
static u1_t IRAM_ATTR readReg (u1_t addr) {
    hal_pin_nss(0);
    hal_spi(addr & 0x7F);
...
static void IRAM_ATTR readBuf (u1_t addr, xref2u1_t buf, u1_t len) {
    hal_pin_nss(0);
    hal_spi(addr & 0x7F);
    for (u1_t i=0; i<len; i++) {
        buf[i] = hal_spi(0x00);
...
static void IRAM_ATTR opmode (u1_t mode) {
...
static void txlora () {
    ...
    syslog(LOG_DEBUG, "%lu: TXMODE, freq=%lu, len=%d, SF=%d, BW=%d, CR=4/%d, IH=%d\n",
    		(unsigned long)os_getTime(), (unsigned long)LMIC.freq, LMIC.dataLen, sf,
