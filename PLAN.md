...
DONE: Review all project code for unchecked user-supplied lengths — Content-Length, packet length fields, or any length arriving from the network or Lua caller used directly in malloc, memcpy, or read without validation against a maximum.

- F1 (httpsrv.c:1060): `atoi(contentlen)+1` integer overflow — if Content-Length is INT_MAX, adding 1 wraps to INT_MIN (negative). The `> HTTP_BUFF_SIZE` clamp silently passes the negative value. `do_gets(pathbuf, INT_MIN, ...)` then performs pointer arithmetic that allows writing far beyond the 1024-byte pathbuf heap buffer (heap overflow). Fixed: split into `int clen = atoi(contentlen)` with a single conditional expression that is positive and strictly less than HTTP_BUFF_SIZE before adding 1; otherwise defaults to HTTP_BUFF_SIZE.
- F2 (mqtt/SocketBuffer.c:189-199): When `malloc(bytes)` fails in the `queue->datalen > 0` branch, the existing `#if __XTENSA__` guard skipped memcpy but still executed `free(queue->buf)` and `queue->buf = NULL`. Back in `Socket_getdata`, `buf = NULL` then `recv(socket, NULL + 0, bytes, 0)` — null pointer dereference crash. Fixed: replaced the `#if __XTENSA__` guarded block with `if (!newmem) return NULL;` so the caller gets NULL and propagates the error safely; the matching `else` branch already had `if (!newbuf) return NULL`.
...
- Replace sdspi_slot_config_t with sdspi_device_config_t and SDSPI_SLOT_CONFIG_DEFAULT() with SDSPI_DEVICE_CONFIG_DEFAULT().
- Replace sdspi_device_config_t.gpio_miso/mosi/sck fields (removed in v5) with host_id (SPI bus number) and gpio_cs only; the bus pins are already configured by the SPI driver.
- Replace esp_vfs_fat_sdmmc_mount() for the SPI path with esp_vfs_fat_sdspi_mount().
- Replace esp_vfs_fat_sdmmc_unmount() (deprecated) with esp_vfs_fat_sdcard_unmount(path, card) in both vfs_fat_umount() and vfs_fat_format().
...
- sd.umount([path])  -- unmount the SD card
- sd.info()          -- return a table with name, type ("SDHC/SDXC" or "SDSC"), and size_mb
- sd.format([path])  -- reformat the FAT filesystem on the card
...
DONE: Enable CONFIG_HTTPD_WS_SUPPORT in sdkconfig

Set CONFIG_HTTPD_WS_SUPPORT=y in sdkconfig (was commented out). This enables httpd_ws_recv_frame, httpd_ws_send_frame_async, and httpd_ws_get_fd_info APIs in esp_http_server.
...
DONE: Add CONFIG_LUA_RTOS_LUA_USE_WEBSOCKET to main/Kconfig

Added config LUA_RTOS_LUA_USE_WEBSOCKET after LUA_RTOS_LUA_USE_COAP in main/Kconfig, depends on LUA_RTOS_LUA_USE_NET, default n.
...
DONE: Create the websocket/ component directory with CMakeLists.txt

Created websocket/CMakeLists.txt (conditionally compiles websocket_server.c and websocket_client.c when CONFIG_LUA_RTOS_LUA_USE_WEBSOCKET is set, REQUIRES esp_http_server tcp_transport sys). Added "websocket" to EXTRA_COMPONENT_DIRS in top-level CMakeLists.txt.
...
DONE: Create websocket/websocket.h with the C-level driver API

Created websocket/include/websocket.h with ws_server_handle_t, ws_client_handle_t, callback typedefs (ws_msg_cb_t, ws_open_cb_t, ws_close_cb_t), server API (ws_server_create/destroy/send/broadcast), and client API (ws_client_create/destroy/send). Also added WS_DRIVER_ID=45 to sys/sys/driver.h.
...
DONE: Implement WebSocket server in websocket/websocket_server.c

Created websocket/websocket_server.c using httpd_start/httpd_register_uri_handler with is_websocket=true. The URI handler dispatches frames: CLOSE triggers on_close and sends a close response; TEXT/BINARY payload is heap-allocated, passed to on_message, then freed. ws_server_send uses httpd_ws_send_frame_async (task-safe). Client FDs are tracked in an array for broadcast support.
...
DONE: Implement WebSocket client in websocket/websocket_client.c

Created websocket/websocket_client.c. Parses ws:// or wss:// URIs to extract host/port/path. Uses esp_transport_tcp_init (or ssl_init for wss://) as base, wraps with esp_transport_ws_init, sets the path, and calls esp_transport_connect. Spawns a FreeRTOS rx task that loops on esp_transport_read, dispatches text/binary frames to on_message, CLOSE to on_close. ws_client_send uses esp_transport_ws_send_raw with TEXT opcode.
...
DONE: Create lua/modules/middleware/websocket.c with the Lua module

Created lua/modules/middleware/websocket.c guarded by CONFIG_LUA_RTOS_LUA_USE_WEBSOCKET. Defines lws_server_t and lws_client_t userdatas each with 3 callback slots (message/open/close). Exposes ws.server(port) and ws.client(uri) top-level constructors; server objects have :on/:send/:broadcast/:close methods; client objects have :on/:send/:close methods. Registered with MODULE_REGISTER_ROM(WS, ws, lws_map, luaopen_ws, 1).
...
DONE: Update lua/CMakeLists.txt to include the WebSocket module

Added modules/middleware/websocket.c to srcs, websocket to PRIV_REQUIRES, and -u luaopen_ws to extra_link_flags in lua/CMakeLists.txt.
...
DONE: Write a Lua test script for WebSocket server and client

Created fs_images/default/examples/lua/websocket.lua. Connects WiFi, starts an echo server on port 8080, then creates a client that connects to ws://127.0.0.1:8080/, sends "hello", prints the echoed reply in on_message, and closes. Also prints the device IP for external wscat/browser testing.
...
API directly (namespace "wifiprov", string keys "ssid" and "pass").  Use nvs_get_str /
...
In sys/drivers/wifi_prov.c, implement wifi_prov_start(ap_ssid, ap_pass).  It must:
  1. Call wifi_setup(WIFI_MODE_AP, ap_ssid, ap_pass, 0,0,0,0,0, 0, 1, 0) to bring up
     a SoftAP on channel 1 with DHCP enabled (IP 192.168.4.1).
  2. Call wifi_start(0) (synchronous).
  3. Retrieve the AP interface IP via wifi_stat() and store it in a static ifconfig_t
     for use by the DNS and HTTP tasks.
  4. Call captivedns_start() (passing a dummy lua_State* NULL workaround or refactor
     captivedns_start to not require lua_State) to start the captive DNS server so
     that all DNS queries resolve to 192.168.4.1.
...
(not the full httpsrv).  For each accepted connection:
  ...
  - GET / : call wifi_scan() to get a list of nearby APs, then respond with a
    self-contained HTML page containing a <form method="POST"> with a <select>
    listing scanned SSIDs, a manual SSID text input, and a password input.
  - POST / : read the Content-Length body, URL-decode the "ssid" and "pass" fields,
    call wifi_prov_save_credentials(), respond with a minimal HTML "Credentials saved,
    rebooting..." page, close the socket, wait 500 ms, then call esp_restart().
...
Define three Lua-callable C functions:
  ...
  lnet_wf_prov_reset(L)   — calls wifi_prov_erase_credentials(), returns nothing
  lnet_wf_prov_has(L)     — returns boolean
...
provisioning logic:
  - If net.wf.prov exists (feature compiled in) and config.wifi is true:
      - If not net.wf.prov.has_credentials() then
            net.wf.prov.start("LuaRTOS-Setup", "")  -- blocks until reboot
        Else
            Load ssid/pass from NVS via nvs.read and override config.data.wifi.ssid
...
Create fs_images/default/examples/lua/wifi-prov.lua demonstrating:
  ...
  - Manually starting the provisioning portal (net.wf.prov.start()).
  - Erasing stored credentials to force re-provisioning (net.wf.prov.reset()).
Include comments explaining the expected user flow (connect phone to "LuaRTOS-Setup",
open any URL in browser, fill in the form).
...
WIFI_AUTH_WPA3_PSK or WIFI_AUTH_WPA2_WPA3_PSK, configure the sta config as follows:
  - Set wifi_config.sta.threshold.authmode to the requested auth mode.
  - Set wifi_config.sta.pmf_cfg.required = (auth_mode == WIFI_AUTH_WPA3_PSK) ? true : false
    and wifi_config.sta.pmf_cfg.capable = true (PMF required for WPA3-only, optional for
    the transition mode).
  - Set wifi_config.sta.sae_pwe_h2e = WPA3_SAE_PWE_BOTH to support both H2E and
    Hunt-and-Peck derivation methods.
For all other auth modes (including 0 / unspecified) leave the fields at their zero-
...
Two call-sites exist outside of net_wifi.inc:
  1. sys/drivers/wifi_prov.c: wifi_prov_start() calls wifi_setup() for AP mode.
     Pass auth_mode = 0 (WIFI_AUTH_OPEN / default) so provisioning behaviour is
     unchanged.
...
In the wifi_auth_map[] table (around line 522) add:
  ...
  { LSTRKEY( "WPA2_WPA3_PSK" ), LINTVAL( WIFI_AUTH_WPA2_WPA3_PSK ) },
...
for auth mode:
  int auth_mode = luaL_optinteger(L, 11, 0);
Pass this value as the new auth_mode argument to the wifi_setup() C call on line 120.
The argument position (11) follows the existing optional channel argument at position 10,
...
Create lua/tests/test_wifi_wpa3.lua that:
  ...
  2. Calls net.wf.setup() again with net.wf.auth.WPA2_WPA3_PSK and asserts no error.
